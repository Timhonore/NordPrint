...
/**
 * The publishable key, read at request time.
 *
 * It is deliberately *not* a `NEXT_PUBLIC_` variable. Next inlines those at
 * build time, and the key is generated by the seed — so a rebuilt storefront
 * paired with a re-seeded database would carry a stale key and every core
 * store call (carts, checkout, orders) would fail with "A valid publishable
 * key is required". Nothing in the browser needs the key: this module only
 * ever runs on the server.
 *
 * `NEXT_PUBLIC_MEDUSA_PUBLISHABLE_KEY` still works as a fallback so an
 * existing deployment keeps running after an upgrade.
 */
...
export interface FetchOptions {
  ...
  cookie?: string;
  /** Customer JWT. Sent as `Authorization: Bearer` — never logged. */
  token?: string;
  signal?: AbortSignal;
}
