...
export class Migration20260823154108 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table if not exists "brand" ("id" text not null, "name" text not null, "handle" text not null, "logo_url" text null, "description" text null, "website_url" text null, "featured" boolean not null default false, "rank" integer not null default 0, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "brand_pkey" primary key ("id"));`);
    ...
    this.addSql(`CREATE INDEX IF NOT EXISTS "IDX_brand_featured" ON "brand" ("featured") WHERE deleted_at IS NULL;`);
  }
